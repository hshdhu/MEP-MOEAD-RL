...
# [SỬA ĐỔI]: Import class PPO_LSTM thay vì PPO
from algorithm.ppo_lstm import PPO_LSTM
import numpy as np
...
def main():
    config = load_config()
    if len(sys.argv) < 2:
        print("Usage: python run_ppo_lstm.py <path_to_env_json>")
        sys.exit(1)
    ...
    # [SỬA ĐỔI]: Đổi tên thư mục lưu kết quả thành PPO_LSTM_... để phân biệt
    result_dir = Path("result") / f"{result_folder} sensors" / f"PPO_LSTM_{timestamp}"
    result_dir.mkdir(parents=True, exist_ok=True)

    snapshot_dir = result_dir / "snapshots"
    snapshot_dir.mkdir(exist_ok=True)

    print(f"PPO_LSTM Output: {result_dir}")

    # Khởi tạo PPO_LSTM với tham số từ config (vẫn dùng config của ppo)
    ppo_params = config.config['algorithm']['ppo']

    # [SỬA ĐỔI]: Khởi tạo bằng class PPO_LSTM
    agent = PPO_LSTM(env, **ppo_params)
    ...
    for i, (path, (f1, f2)) in enumerate(pareto):
        solutions_data.append({
            'algorithm': 'PPO_LSTM',  # [SỬA ĐỔI]: Ghi nhận thuật toán là PPO_LSTM vào file JSON
            'id': i,
            'exposure': -f1,
            'length': f2,
            'path': [[p.x, p.y] for p in path]
    ...
    print(f"PPO_LSTM finished in {run_time:.2f}s. Found {len(solutions_data)} solutions.")
